...
class ImageUtil {
  static   Future<File> rotate(String imagePath, num angle) async {
  ...
  static Future<File> writeBytesToPngFile(Uint8List pngBytes) async {
  ...
  static Future<File>  jpegToPng(jpgPath) async {
    final image = decodeImage(File(jpgPath).readAsBytesSync())!;
    final dDayLocal = DateTime.now();
    final Directory directory = await getTemporaryDirectory();
    String fileName = "${dDayLocal.year}${dDayLocal.month}${dDayLocal.day}${dDayLocal.hour}${dDayLocal.minute}${dDayLocal.second}${dDayLocal.millisecond}.png";
    return File("${directory.path}/$fileName")..writeAsBytesSync(encodeJpg(image));
